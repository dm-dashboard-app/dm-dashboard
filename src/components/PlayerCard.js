...
import SpellWorkflowPanel, {
  ConcentrationSpellPickerModal,
} from "./SpellWorkflowPanel";
import {
  readNumberField,
  ABILITY_KEYS,
  getAbilityScores,
...
function resolveDisplayedValues(resource, state) {
  ...
  if (
    ...
    const current =
      rawCurrent === null || rawCurrent <= 0 ? fallbackCurrent : rawCurrent;
...
export default function PlayerCard({
  ...
  const pendingConDc = concentration
    ? (state?.concentration_check_dc ?? null)
    : null;
  ...
  const hitDiceResource = resourceConfigs.find(
    (resource) => resource.id === "hit-dice" || resource.label === "Hit Dice",
  );
  const otherResources = resourceConfigs.filter(
    (resource) => resource !== hitDiceResource,
  );
  ...
  const spellSaveDc =
    profile?.spell_save_dc || getFinalSpellSaveDC(profile || {});
  const spellAttackBonus =
    profile?.spell_attack_bonus || getFinalSpellAttackBonus(profile || {});
  const armorClass = getFinalArmorClass(profile || {}, state || {});
  const passivePerception = profile
    ? 10 + (profile.skill_perception ?? 0)
    : null;
  ...
  }, [state?.concentration_spell_id, combatant?.concentration_spell_id]);
  ...
  async function submitTempHp(value) {
  ...
  async function submitBonusHp(value) {
    if (!state || !canRestore) return;
    ...
    await supabase
      .from("player_encounter_state")
      .update({ max_hp_override: overrideVal, current_hp: newHp })
      .eq("id", state.id);
    setHpAdjustModal(null);
    onUpdate();
  }
  async function resetMaxHp() {
    ...
    await supabase
      .from("player_encounter_state")
      .update({ max_hp_override: null, current_hp: newHp })
      .eq("id", state.id);
    onUpdate();
  }
  ...
  async function handleConPass() {
    ...
    await supabase
      .from("player_encounter_state")
      .update({ concentration_check_dc: null })
      .eq("id", state.id);
    onUpdate();
  }
  ...
  const initials = (combatant?.name || "?")
    ...
    .slice(0, 2);
  const barBaseStyle = {
    minHeight: "var(--pc-status-bar-min-height)",
    width: "100%",
    padding: "var(--pc-status-bar-padding)",
    borderRadius: "var(--pc-status-bar-radius)",
    display: "flex",
    alignItems: "center",
    justifyContent: "space-between",
    gap: "var(--pc-status-bar-gap)",
    fontWeight: 700,
  };
  return (
    <>
      <div className={`player-card ${densityClass}`}>
        ...
        <div className="card-body player-card-body-reflow">
          <div>
            <span
              className="card-name"
              style={{
                fontSize: "var(--pc-name-size)",
                lineHeight: "var(--pc-name-line-height)",
                minWidth: 0,
                maxWidth: "100%",
                whiteSpace: "nowrap",
                overflow: "hidden",
          ...
          <button
            ...
            style={{
              ...barBaseStyle,
              border: reactionUsed
                ? "1px solid rgba(191,59,74,.55)"
                : "1px solid rgba(72,225,124,.55)",
              background: reactionUsed
                ? "rgba(58,26,31,.88)"
                : "rgba(26,58,42,.88)",
          ...
          >
            ...
            <span className="player-concentration-value">
              {concentrationText}
            </span>
          </button>
          {pendingConDc !== null && (
            <div className="con-check-banner">
              ...
              <div className="con-check-actions">
                <button className="con-check-pass" onClick={handleConPass}>
                  Passed
                </button>
                <button className="con-check-fail" onClick={handleConFail}>
                  Failed
                </button>
              </div>
            </div>
          ...
          <div className="player-card-four-up">
            ...
            <InfoStatBox
              label="Spell DC"
              value={spellSaveDc > 0 ? spellSaveDc : "—"}
            />
            <InfoStatBox
              label="Spell ATK"
              value={spellAttackBonus ? formatModifier(spellAttackBonus) : "—"}
            />
          </div>
          <div className="player-hp-hero-wrap">
            <div className="player-hp-hero-track">
              ...
              {bonusCurrentHp > 0 && (
                <div
                  className="player-hp-hero-temp"
                  style={{
                    left: `${bonusFillLeftPercent}%`,
                    width: `${bonusFillPercent}%`,
                    background: BONUS_HP_COLOR,
                    opacity: 0.72,
                  }}
                />
              )}
              {tempHp > 0 && (
                <div
                  className="player-hp-hero-temp"
                  style={{
                    left: `${tempFillLeftPercent}%`,
                    width: `${tempFillPercent}%`,
                    background: TEMP_HP_COLOR,
                    opacity: 0.72,
                  }}
                />
              )}
              <div className="player-hp-hero-text">
                {hp} / {maxHp}
          ...
          {canEdit && (
            <DmgHealRow onDamage={handleApplyDamage} onHeal={handleApplyHeal} />
          )}
          {hitDiceResource ? (
            <HitDiceRow
              resource={hitDiceResource}
              state={state}
              readOnly={!canRestore}
          ...
          {canRestore && (
            <div className="player-bonus-temp-row">
              <button
                className="btn btn-ghost dm-card-adjustment-btn dm-card-adjustment-btn--gold"
                onClick={() =>
                  setHpAdjustModal({
                    type: "bonus",
                    title: "Grant bonus max HP by how much?",
                    defaultValue: "0",
                  })
                }
              >
              ...
              <button
                className="btn btn-ghost dm-card-adjustment-btn dm-card-adjustment-btn--blue"
                onClick={() =>
                  setHpAdjustModal({
                    type: "temp",
                    title: "Set temporary HP to:",
                    defaultValue: String(tempHp || 0),
                  })
                }
              >
                Temp HP
              ...
              {maxHpOverride !== null ? (
                <button
                  className="btn btn-ghost dm-card-adjustment-btn"
                  onClick={resetMaxHp}
                >
                  Reset HP
                </button>
          ...
          <SectionGrid title="Ability Scores" className="player-ability-grid">
            {ABILITY_KEYS.map((key) => (
              <div key={key} className="player-ability-cell">
                ...
                <span className="player-ability-score">
                  {abilityScores[key]}
                </span>
                <span className="player-ability-mod">
          ...
          <SectionGrid title="Saving Throws" className="player-save-grid">
            {ABILITY_KEYS.map((key) => (
              <div key={key} className="player-save-cell">
                ...
                <span className="player-save-value">
                  {formatModifier(saveTotals[key])}
                </span>
              </div>
            ))}
          </SectionGrid>
          {!readOnly && (
            <div className="form-row player-card-actions-row">
              <button
                className="btn btn-ghost player-card-skills-btn"
      ...
      {!readOnly && profile && (
        <SkillsModal
          ...
          profile={profile}
          title={`${combatant?.name || "Character"} Skills`}
        />
      )}
      {showSpells && profile && (
        <SpellWorkflowPanel
          ...
          title={`${combatant?.name || "Character"} Spells`}
          subtitle={
            role === "dm"
              ? "Browse and inspect this player's spell lists."
              : "Browse your spell lists and inspect spell details."
          }
        />
      )}
      {showConPicker && profile && (
        <ConcentrationSpellPickerModal
          open={showConPicker}
          profile={profile}
          state={state}
      ...
      {hpAdjustModal && (
        <NumericHpModal
          title={hpAdjustModal.title}
          defaultValue={hpAdjustModal.defaultValue}
          onClose={() => setHpAdjustModal(null)}
          onSubmit={(value) =>
            hpAdjustModal.type === "bonus"
              ? submitBonusHp(value)
              : submitTempHp(value)
          }
        />
...
function InfoStatBox({ label, value, highlight = "" }) {
  return (
    <div
      className={`player-info-stat-box ${highlight ? `player-info-stat-box--${highlight}` : ""}`}
    >
...
function HitDiceRow({ resource, state, readOnly, onUpdateFields }) {
  ...
  async function adjust(delta) {
    ...
    await onUpdateFields({
      [resource.currentKey]: next,
      ...(resource.maxKey && max !== null ? { [resource.maxKey]: max } : {}),
    });
  }
  return (
    <div className="player-hit-dice-row">
      <div className="player-hit-dice-copy">
        ...
        {resource.meta ? (
          <span className="player-hit-dice-meta">{resource.meta}</span>
        ) : null}
      </div>
      <div className="player-hit-dice-controls">
        {!readOnly ? (
          <button
            ...
            style={{ minWidth: 28, minHeight: 28, fontSize: 13 }}
            onClick={() => adjust(-1)}
          >
            −
          </button>
        ) : null}
        <span className="player-hit-dice-value">
          {current}
          {max !== null ? ` / ${max}` : ""}
        </span>
        {!readOnly ? (
...
function NumericHpModal({ title, defaultValue, onClose, onSubmit }) {
...
function readyStyle(ready) {
  return ready
    ? {
        borderColor: "rgba(72,225,124,.55)",
        background: "rgba(26,58,42,.88)",
        color: "var(--accent-green)",
      }
    : {
        borderColor: "rgba(191,59,74,.55)",
        background: "rgba(58,26,31,.88)",
        color: "var(--accent-red)",
      };
}
function ResourceRow({ resource, state, readOnly, onUpdateFields }) {
  const accentColor = "var(--accent-blue)";
  const accentFill = "var(--accent-blue)";
  if (resource.type === "toggle") {
    ...
    async function handleToggle() {
      ...
      const nextRaw =
        resource.toggleMode === "available" ? nextReady : !nextReady;
      await onUpdateFields({ [resource.boolKey]: nextRaw });
    }
    const styles = readyStyle(toggleState.ready);
    return (
      <div className="player-resource-row">
        <div className="player-resource-copy">
          ...
          {resource.meta ? (
            <span className="player-resource-meta">{resource.meta}</span>
          ) : null}
        </div>
  ...
  if (resource.type === "counter") {
    ...
    if ((max ?? 0) <= 1) {
      const ready = (current ?? 0) > 0;
      const styles = readyStyle(ready);
      return (
        <div className="player-resource-row">
          <div className="player-resource-copy">
            <span className="player-resource-label">{resource.label}</span>
      ...
      <div className="player-resource-row">
        <div className="player-resource-copy">
          ...
          {resource.meta || resource.displaySuffix ? (
            <span className="player-resource-meta">
              {[resource.meta, resource.displaySuffix]
                .filter(Boolean)
                .join(" • ")}
            </span>
        ...
        <div className="player-resource-controls">
          ...
          <span className="player-resource-value">
            {current}
            {max !== null ? ` / ${max}` : ""}
          </span>
          {!readOnly && (
  ...
  if (safeMax <= 1) {
    const ready = safeCurrent > 0;
    const styles = readyStyle(ready);
    async function toggleSingle() {
      if (readOnly) return;
      const updates = { [resource.currentKey]: ready ? 0 : 1 };
    ...
    return (
      <div className="player-resource-row">
        <div className="player-resource-copy">
          <span className="player-resource-label">{resource.label}</span>
          {[resource.meta, `${safeCurrent}/${safeMax}`].filter(Boolean).length >
          0 ? (
            <span className="player-resource-meta">
              {[resource.meta, `${safeCurrent}/${safeMax}`]
                .filter(Boolean)
                .join(" • ")}
            </span>
          ) : null}
