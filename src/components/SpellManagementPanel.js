...
function modeHelpText(mode) {
  if (mode === 'wizard') return 'Wizard mode: learn spells into the spellbook, then prepare from learned spells. Cantrips are learned only.';
  if (mode === 'prepared') return 'Prepared mode: prepare daily from the legal class list. Cantrips are added/removed rather than prepared.';
  if (mode === 'known') return 'Known-caster mode: maintain a learned spell pool. Cantrips are learned too.';
...
function componentsText(components = [], material = '') {
...
function mapApiSpell(detail = {}) {
  const classTags = (detail.classes || []).map(entry => entry.name).map(normalizeClassName).filter(Boolean);
  ...
  return {
    ...
    components_text: componentsText(detail.components, detail.material),
...
async function upsertSpell(payload) {
  return supabase.from('spells').upsert(payload, { onConflict: 'external_key' });
}
...
async function importSrdSpells(setProgress) {
  const listResponse = await fetch(`${API_ROOT}/spells`);
  if (!listResponse.ok) throw new Error('Failed to fetch SRD spell index');
  ...
  for (let i = 0; i < results.length; i += batchSize) {
    ...
    setProgress?.(`Imported ${Math.min(i + batch.length, results.length)} / ${results.length} SRD spells…`);
  }
}
...
async function ensurePlayerSpell(playerId, spellId, patch = {}) {
  ...
  if (existing?.id) {
    return supabase.from('profile_player_spells').update(patch).eq('id', existing.id);
  }
...
async function ensureMonsterSpell(monsterId, spellId, patch = {}) {
  ...
  if (existing?.id) {
    return supabase.from('profile_monster_spells').update(patch).eq('id', existing.id);
  }
...
function SpellRow({ spell, assigned, targetType, assignmentMode, selectedTargetId, onAssign, onPrepare, onEditHomebrew }) {
  ...
  if (targetType === 'player') {
    if (assignmentMode === 'wizard') {
      buttons = (
        <>
          <button className="btn btn-ghost" style={{ fontSize: 11, padding: '2px 8px' }} onClick={() => onAssign(spell, !isKnown)}>{isKnown ? 'Forget' : 'Learn'}</button>
          {!isCantrip && (
            <button className="btn btn-ghost" style={{ fontSize: 11, padding: '2px 8px' }} onClick={() => onPrepare(spell, !isPrepared)} disabled={!isKnown && !isPrepared}>{isPrepared ? 'Unprepare' : 'Prepare'}</button>
          )}
        </>
      );
      ...
      buttons = (
        <button className="btn btn-ghost" style={{ fontSize: 11, padding: '2px 8px' }} onClick={() => onAssign(spell, !isKnown)}>{isKnown ? 'Forget' : (isCantrip ? 'Add Cantrip' : 'Learn')}</button>
      );
    }
  } else {
    buttons = (
      <>
        <button className="btn btn-ghost" style={{ fontSize: 11, padding: '2px 8px' }} onClick={() => onAssign(spell, !isKnown)}>{isKnown ? 'Unassign' : 'Assign'}</button>
        {!isCantrip && selectedTargetId && isKnown && <button className="btn btn-ghost" style={{ fontSize: 11, padding: '2px 8px' }} onClick={() => onPrepare(spell, !isPrepared)}>{isPrepared ? 'Unprep' : 'Prepare'}</button>}
      </>
    );
  }
    ...
    <div style={{ border: '1px solid var(--border)', borderRadius: 8, padding: '8px 10px', display: 'flex', flexDirection: 'column', gap: 6 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', gap: 8, alignItems: 'flex-start' }}>
        <div style={{ minWidth: 0 }}>
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', alignItems: 'center' }}>
            ...
            <span style={{ fontSize: 10, color: 'var(--text-muted)' }}>{spell.level === 0 ? 'Cantrip' : `Level ${spell.level}`}</span>
          ...
          {spell.description && <div style={{ fontSize: 11, color: 'var(--text-secondary)', marginTop: 2, whiteSpace: 'pre-wrap' }}>{spell.description.slice(0, 220)}{spell.description.length > 220 ? '…' : ''}</div>}
        </div>
      ...
      {(isKnown || isPrepared) && (
        <div style={{ fontSize: 10, color: 'var(--accent-blue)' }}>
          {isKnown ? 'Assigned' : ''}{isKnown && isPrepared ? ' • ' : ''}{isPrepared ? 'Prepared' : ''}
        </div>
      )}
    </div>
  );
  ...
  return (
    <div className="panel" style={{ marginTop: 12 }}>
      ...
      <div className="form-row" style={{ gap: 8 }}>
        <div className="form-group" style={{ flex: 1 }}><label className="form-label">Level</label><input className="form-input" type="number" min={0} max={9} value={form.level} onChange={e => setForm(current => ({ ...current, level: parseInt(e.target.value || '0', 10), is_cantrip: parseInt(e.target.value || '0', 10) === 0 }))} /></div>
...
export default function SpellManagementPanel() {
  ...
  async function saveHomebrew() {
    ...
    const { error } = await upsertSpell(payload);
  ...
  async function handleAssign(spell, nextKnown) {
    ...
    if (targetType === 'player') {
      if (nextKnown) {
        const patch = assignmentMode === 'prepared' && spell.level > 0 ? { is_known: true, is_prepared: true } : { is_known: true };
        await ensurePlayerSpell(selectedTargetId, spell.id, patch);
      } else {
        await removePlayerSpell(selectedTargetId, spell.id);
      }
    } else {
      if (nextKnown) await ensureMonsterSpell(selectedTargetId, spell.id, { is_known: true, is_prepared: spell.level > 0 });
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12, marginTop: 12 }}>
      <div className="panel">
        ...
        <div className="form-row" style={{ flexWrap: 'wrap', gap: 8, marginBottom: 8 }}>
          ...
          <select className="form-input" style={{ width: 120 }} value={sourceFilter} onChange={e => setSourceFilter(e.target.value)}>{SOURCE_OPTIONS.map(option => <option key={option} value={option}>{option === 'all' ? 'All Sources' : option}</option>)}</select>
          <select className="form-input" style={{ width: 120 }} value={String(levelFilter)} onChange={e => setLevelFilter(e.target.value === 'all' ? 'all' : Number(e.target.value))}>{LEVEL_OPTIONS.map(option => <option key={String(option)} value={String(option)}>{option === 'all' ? 'All Levels' : option === 0 ? 'Cantrip' : `Level ${option}`}</option>)}</select>
        ...
        <div className="form-row" style={{ flexWrap: 'wrap', gap: 8, marginBottom: 8 }}>
          <select className="form-input" style={{ width: 140 }} value={targetType} onChange={e => { const nextType = e.target.value; setTargetType(nextType); setSelectedTargetId(nextType === 'player' ? (players[0]?.id || '') : (monsters[0]?.id || '')); }}>{[{ value: 'player', label: 'Players' }, { value: 'monster', label: 'Monsters/NPCs' }].map(option => <option key={option.value} value={option.value}>{option.label}</option>)}</select>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {filteredSpells.map(spell => <SpellRow key={spell.id} spell={spell} assigned={assignments[spell.id]} targetType={targetType} assignmentMode={assignmentMode} selectedTargetId={selectedTargetId} onAssign={handleAssign} onPrepare={handlePrepare} onEditHomebrew={openEditHomebrew} />)}
