...
import { derivePlayerProfileDefaults, formatHitDiceSummary, formatStandardSpellSlotsSummary } from '../utils/classResources';
...
function classSummary(entity = {}) {
  ...
  if (entity.class_name) {
    parts.push(
      [entity.class_name, entity.class_level ? entity.class_level : null, entity.subclass_name ? `— ${entity.subclass_name}` : null].filter(Boolean).join(' ')
    );
  }

  if (entity.class_name_2) {
    parts.push(
      [entity.class_name_2, entity.class_level_2 ? entity.class_level_2 : null, entity.subclass_name_2 ? `— ${entity.subclass_name_2}` : null].filter(Boolean).join(' ')
    );
  }
...
export default function ManagementScreens({ onEncounterCreated, currentEncounter = null, displayToken = null, joinCodes = [], onToggleEditMode = null, onGenerateDisplayToken = null, onRevokeDisplayToken = null, onFrontScreen = null, onSignOut = null }) {
  ...
  useEffect(() => {
    if (!currentEncounter && tab === 'session') setTab('players');
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16, marginTop: 12 }}>
      <div className="panel session-subpanel">
        ...
        <div className="session-meta-grid">
          ...
          <div className="session-meta-item"><span className="session-meta-label">Player Edit Mode</span><span className="session-meta-value">{currentEncounter?.player_edit_mode ? 'On' : 'Off'}</span></div>
        </div>
        <div className="form-row" style={{ marginTop: 12, flexWrap: 'wrap' }}>
          {onToggleEditMode && <button className="btn btn-ghost" onClick={onToggleEditMode}>{currentEncounter?.player_edit_mode ? 'Lock Player Editing' : 'Enable Player Editing'}</button>}
      ...
      <div className="panel session-subpanel">
        ...
        {displayToken ? (
          ...
          <div className="form-row" style={{ flexWrap: 'wrap' }}>{onGenerateDisplayToken && <button className="btn btn-ghost" onClick={onGenerateDisplayToken}>Generate Display Token</button>}</div>
        )}
      </div>
      <div className="panel session-subpanel">
...
function NumInput({ value, onChange, ...props }) {
  ...
  return <input {...props} className="form-input" type="number" value={draft} onChange={e => setDraft(e.target.value)} onBlur={() => { const n = parseInt(draft, 10); const final = Number.isNaN(n) ? 0 : n; setDraft(final === 0 ? '' : String(final)); onChange(final); }} />;
}

function SelectField({ value, onChange, options }) {
  return <select className="form-input" value={value || ''} onChange={e => onChange(e.target.value)}>{options.map(opt => <option key={opt || 'blank'} value={opt}>{opt || '—'}</option>)}</select>;
}
...
function DerivedValueField({ label, value, helpText = '' }) {
  return (
    <div className="form-group" style={{ flex: 1 }}>
      ...
      <div className="form-input" style={{ display: 'flex', alignItems: 'center', color: 'var(--text-primary)', fontWeight: 600 }}>{value || '—'}</div>
      {helpText ? <div style={{ marginTop: 4, fontSize: 11, color: 'var(--text-muted)' }}>{helpText}</div> : null}
    </div>
  );
}
...
function PlayerProfileForm({ initial, onSave, onCancel }) {
  const [f, setF] = useState({ name: '', max_hp: 10, ac: 10, initiative_mod: 0, class_name: '', subclass_name: '', class_level: 1, class_name_2: '', subclass_name_2: '', class_level_2: 0, ancestry_name: '', feat_lucky: false, feat_relentless_endurance: false, hit_die_size: 8, hit_dice_max: 1, save_str: 0, save_dex: 0, save_con: 0, save_int: 0, save_wis: 0, save_cha: 0, spell_save_dc: 8, spell_attack_bonus: 0, skill_perception: 0, skill_insight: 0, skill_investigation: 0, skill_survival: 0, slots_max_1: 0, slots_max_2: 0, slots_max_3: 0, slots_max_4: 0, slots_max_5: 0, slots_max_6: 0, slots_max_7: 0, slots_max_8: 0, slots_max_9: 0, wildshape_enabled: false, portrait_url: '', ...initial });
  ...
  return (
    <div className="profile-form">
      ...
      <div className="form-row" style={{ flexWrap: 'wrap' }}>
        <DerivedValueField label="Hit Dice" value={hitDiceSummary || '—'} helpText="Derived automatically from class selection and multiclass levels." />
        <DerivedValueField label="Spell Slots" value={spellSlotsSummary || 'No standard slots'} helpText="Derived automatically from multiclass caster level. Warlock pact slots remain separate in encounter state." />
      </div>
      ...
      <div className="saves-grid">{['str','dex','con','int','wis','cha'].map(s => <div key={s} className="form-group"><label className="form-label">{s.toUpperCase()}</label><NumInput value={f[`save_${s}`]} onChange={v => set(`save_${s}`, v)} /></div>)}</div>
      <div className="panel-title" style={{ marginTop: 12 }}>Skills</div>
      {['perception','insight','investigation','survival'].map(sk => <Field key={sk} label={sk.charAt(0).toUpperCase() + sk.slice(1)}><NumInput value={f[`skill_${sk}`] ?? 0} onChange={v => set(`skill_${sk}`, v)} /></Field>)}
      <label className="checkbox-row" style={{ marginTop: 12 }}><input type="checkbox" checked={f.wildshape_enabled} onChange={e => set('wildshape_enabled', e.target.checked)} /><span>Wild Shape Enabled</span></label>
      <div className="form-row" style={{ marginTop: 16 }}><button className="btn btn-primary" onClick={() => onSave(derivedProfile)} disabled={!f.name || uploading}>Save</button><button className="btn btn-ghost" onClick={onCancel}>Cancel</button></div>
    </div>
  );
}
