      ...
      {profiles.map((profile) => (
        <div key={profile.id} className="manage-row manage-row--list-item">
          ...
          <div className="manage-row-actions">
            ...
            <button className="btn btn-danger" onClick={() => remove(profile.id)}>Delete</button>
          </div>
        </div>
      ))}
...
function MonsterTemplateManager() {
  ...
  if (editing !== null) return <MonsterForm initial={editing} onSave={save} onCancel={() => setEditing(null)} />;
      ...
      {filtered.map((monster) => (
        <div key={monster.id} className="manage-row manage-row--list-item">
          ...
          <div className="manage-row-actions">
            ...
            {monster.archived
              ? <button className="btn btn-ghost" onClick={() => setArchived(monster.id, false)}>Restore</button>
              : <button className="btn btn-ghost" onClick={() => setArchived(monster.id, true)}>Archive</button>}
            <button className="btn btn-danger" onClick={() => remove(monster.id)}>Delete</button>
          </div>
        </div>
      ))}
