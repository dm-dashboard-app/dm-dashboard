...
export default function WildShapeBlock({ state, readOnly, canRestore, onUpdate }) {
  ...
  const wildShapeMaxUses = state?.wildshape_uses_max ?? 2;
  const [localUses, setLocalUses] = useState(state.wildshape_uses_remaining ?? wildShapeMaxUses);
  ...
  useEffect(() => {
    setLocalUses(state.wildshape_uses_remaining ?? wildShapeMaxUses);
    setLocalHp(null);
    setSelectedFormId(state.wildshape_form_id || '');
  }, [state.wildshape_uses_remaining, state.wildshape_form_id, state.wildshape_hp_current, wildShapeMaxUses]);
  ...
  async function handleRevert() {
    ...
    await supabase
      ...
      .update({
        ...
        wildshape_form_id: null,
        wildshape_hp_current: null,
      })
      .eq('id', state.id);
    onUpdate();
  }
    ...
    <div className="wildshape-block">
      <div className="wildshape-header">
        ...
        <div className="wildshape-uses">
          ...
          <span className="wildshape-uses-count">{localUses} / {wildShapeMaxUses} uses</span>
          {canRestore && (
            <button className="exh-btn" onClick={() => adjustUses(1)} disabled={localUses >= wildShapeMaxUses}>+</button>
          )}
        </div>
      </div>
      ...
      {!state.wildshape_active && !readOnly && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {forms.length === 0 ? (
            ...
            <>
              <select className="form-input" value={selectedFormId} onChange={e => setSelectedFormId(e.target.value)}>
              ...
              <button className="btn btn-success" onClick={handleActivate} disabled={!selectedFormId || localUses <= 0}>
