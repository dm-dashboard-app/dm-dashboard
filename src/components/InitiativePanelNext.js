...
function StatBox({ label, value, visible = true, accent = 'var(--accent-blue)', onClick = null, squareBand = false }) {
  ...
  return (
    <button
      ...
      disabled={!clickable}
      style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 1, minHeight: squareBand ? 30 : 24, height: squareBand ? '100%' : 'auto', width: '100%', borderRadius: 10, border: `1px solid ${accent}55`, background: 'rgba(143,108,244,0.12)', color: 'var(--text-primary)', padding: '1px 2px', textAlign: 'center', cursor: clickable ? 'pointer' : 'default' }}>
...
function InitiativeRow({ combatant, playerState, isActive, isNextUp, isDM, isDisplay, onUpdate, encounterId, myCombatantId, inventoryItems = [] }) {
  ...
  const cardBg = isActive ? 'rgba(143,108,244,0.06)' : isNextUp ? 'rgba(240,180,41,0.05)' : 'var(--bg-panel-2)';
