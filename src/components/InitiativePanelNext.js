...
function InitiativeHeroHpBar({ current, max, tempHp = 0, bonusMaxHp = 0 }) {
  ...
  return (
    <div style={{ position: 'relative', height: 30, background: 'var(--bg-panel-3)', borderRadius: 12, overflow: 'hidden', border: '1px solid var(--border)' }}>
      ...
      <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '0 8px', fontSize: 14, fontWeight: 800, color: '#fff', textShadow: '0 1px 2px rgba(0,0,0,.5)' }}>
...
function FullWidthStatusBar({ label, value, active = false, onClick = null, accent = 'var(--accent-blue)' }) {
  const clickable = typeof onClick === 'function';
  return (
    <button type="button" onClick={clickable ? onClick : undefined} disabled={!clickable} style={{ minHeight: 30, width: '100%', padding: '5px 9px', borderRadius: 12, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10, fontWeight: 700, border: active ? `1px solid ${accent}` : '1px solid var(--border)', background: active ? `${accent}22` : 'var(--bg-panel-3)', color: active ? accent : 'var(--text-primary)', cursor: clickable ? 'pointer' : 'default' }}>
      <span style={{ fontSize: 9, textTransform: 'uppercase', letterSpacing: '.06em' }}>{label}</span>
      <span style={{ fontSize: 12, fontWeight: 800, textAlign: 'right', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{value}</span>
    </button>
  );
}
  ...
  return (
    <>
      <div className={`initiative-row initiative-row-density ${densityClass} ${isActive ? 'active-turn' : ''} ${isNextUp ? 'initiative-row--next-up' : ''}`} style={{ display: 'block', borderColor: topBorder, background: cardBg, borderRadius: 16 }}>
        <div className="initiative-row-top-grid" style={{ display: 'grid', gridTemplateColumns: '46px minmax(0, 1fr) 46px', alignItems: 'stretch' }}>
        ...
        <div className="initiative-row-body" style={{ marginTop: 0, display: 'flex', flexDirection: 'column' }}>
          ...
          {showBottomMeta && <div className="initiative-bottom-meta" style={{ display: 'grid', gridTemplateColumns: 'repeat(3, minmax(0, 1fr))', alignItems: 'center', marginTop: 1 }}><MetaPill>PP {passivePerception ?? '—'}</MetaPill><MetaPill>Spell DC {spellSave ?? '—'}</MetaPill><MetaPill>Spell ATK {spellAttack !== null && spellAttack !== undefined && spellAttack !== '' ? `${spellAttack > 0 ? '+' : ''}${spellAttack}` : '—'}</MetaPill></div>}
        </div>
      </div>
