...
import {
  ...
  CONDITION_COLOURS,
  DAMAGE_TYPES,
} from "./initiative/initiativeConstants";
import {
  compactObject,
  formatClassLine,
  getDisplayOrderedCombatants,
  logCombat,
  nextZeroHpConditions,
  sortCombatants,
  toInt,
} from "./initiative/initiativeUtils";
...
function useDensityTier(role) {
  ...
  if ((role === "dm" || role === "player") && isPhoneViewport) return "phone";
  return "default";
}
...
function InitiativeHeroHpBar({ current, max, tempHp = 0, bonusMaxHp = 0 }) {
  ...
  const hpPct =
    safeMax > 0 ? Math.max(0, Math.min(100, (safeCurrent / safeMax) * 100)) : 0;
  const barColor = resolveThresholdColor(hpPct);

  return (
    <div
      className="initiative-hero-hp-bar"
      style={{
        position: "relative",
        height: "var(--init-hp-height, 34px)",
        background: "var(--bg-panel-3)",
        borderRadius: "var(--init-hp-radius, 14px)",
        overflow: "hidden",
    ...
    >
      ...
      <div
        style={{
          ...
          padding: "var(--init-hp-text-padding, 0 10px)",
          fontSize: "var(--init-hp-font-size, 16px)",
          fontWeight: 800,
          color: "#fff",
...
function StatBox({
  ...
  if (!visible) return <div style={{ minHeight: "var(--init-stat-min-height, 38px)" }} />;
  ...
  return (
    <button
      ...
      style={{
        ...
        alignItems: "center",
        justifyContent: squareBand ? "space-between" : "center",
        gap: squareBand
          ? "var(--init-stat-gap-square, 3px)"
          : "var(--init-stat-gap, 1px)",
        minHeight: squareBand
          ? "var(--init-stat-square-min-height, 52px)"
          : "var(--init-stat-min-height, 38px)",
        height: squareBand ? "100%" : "auto",
        width: "100%",
        borderRadius: "var(--init-stat-radius, 12px)",
        border: `1px solid ${accent}55`,
        background: "rgba(74,158,255,0.12)",
        color: "var(--text-primary)",
        padding: squareBand
          ? "var(--init-stat-square-padding, 5px 2px)"
          : "var(--init-stat-padding, 3px 2px)",
        textAlign: "center",
    ...
    >
      <span
        style={{
          fontSize: "var(--init-stat-label-size, 8px)",
      ...
      <span
        style={{
          fontSize: "var(--init-stat-value-size, 16px)",
          fontWeight: 800,
          lineHeight: 1,
        }}
      >
...
function FullWidthStatusBar({
  ...
  return (
    <button
      ...
      disabled={!clickable}
      style={{
        minHeight: "var(--init-status-min-height, 34px)",
        width: "100%",
        padding: "var(--init-status-padding, 7px 11px)",
        borderRadius: "var(--init-status-radius, 12px)",
        display: "flex",
        alignItems: "center",
        justifyContent: "space-between",
        gap: "var(--init-status-gap, 10px)",
        fontWeight: 700,
        border: active ? `1px solid ${accent}` : "1px solid var(--border)",
        background: active ? `${accent}22` : "var(--bg-panel-3)",
    ...
    >
      <span
        style={{
          fontSize: "var(--init-status-label-size, 10px)",
          textTransform: "uppercase",
          letterSpacing: ".06em",
        }}
      >
      ...
      <span
        style={{
          fontSize: "var(--init-status-value-size, 13px)",
          fontWeight: 800,
          textAlign: "right",
          overflow: "hidden",
  ...
  return (
    <span
      style={{
        ...
        justifyContent: "center",
        width: "100%",
        padding: "var(--init-meta-pill-padding, 5px 8px)",
        borderRadius: 999,
        border: "1px solid var(--border)",
        background: "var(--bg-panel-3)",
        fontSize: "var(--init-meta-pill-font-size, 10px)",
        fontWeight: 700,
...
function InitiativeNumberModal({
  open,
  title,
  defaultValue,
  onClose,
...
export default function InitiativePanelNext({
  encounter,
  combatants,
  playerStates = [],
  role,
  ...
  const densityTier = useDensityTier(role);
  const densityClass = `density-tier density-tier--${densityTier}`;
  const sortedOriginal = sortCombatants(combatants);
  const activeTurnIndex = encounter?.turn_index ?? 0;
  const displayOrdered = getDisplayOrderedCombatants(
    sortedOriginal,
    activeTurnIndex,
  );
  ...
  return (
    <div className={`panel ${densityClass}`}>
      <div className="panel-title">
        {isDisplay ? "Initiative Feed" : "Initiative Order"}
      </div>
      <div className="initiative-list initiative-list--density">
        ...
        {displayOrdered.map((combatant, displayIndex) => {
          const playerState = playerStates.find(
            (s) => s.combatant_id === combatant.id,
          );
          const originalIndex = sortedOriginal.findIndex(
...
function InitiativeRow({
  ...
  useEffect(() => {
    ...
    async function loadSpellName() {
      const spellId =
        playerState?.concentration_spell_id ??
  ...
  const concentration =
    playerState?.concentration ?? combatant?.concentration ?? false;
  ...
  const passivePerception =
    isPC && playerState?.profiles_players
      ? 10 + (playerState.profiles_players.skill_perception ?? 0)
      : (combatant.passive_perception ?? null);
  ...
  const showEnemyHp =
    isEnemy && enemyHpCurrent !== null && enemyHpMax !== null && isDM;
  const showHpBar = isPC
    ? showPcHp
    : isNPC
      ? (isDM || isDisplay) && showNpcHp
      : isDM && showEnemyHp;
  const pcBloodied =
    isPC &&
    pcHpCurrent !== null &&
    pcHpMax !== null &&
    pcHpCurrent > 0 &&
    pcHpCurrent <= Math.floor(pcHpMax / 2);
  const enemyBloodied = isNonPC && combatant.public_status === "BLOODIED";
  const rxUsed = isPC ? reactionUsed : enemyReactionUsed;
  const canToggleReaction =
    isDM || (!isDM && isPC && myCombatantId && combatant.id === myCombatantId);
  const pcProfile = playerState?.profiles_players || {};
  const armorClass = isPC
    ? getFinalArmorClass(pcProfile, playerState || {})
    : (combatant.ac ?? "—");
  const spellSave = isPC
    ? pcProfile?.spell_save_dc || getFinalSpellSaveDC(pcProfile || {})
    : (combatant.spell_save_dc ?? null);
  const spellAttack = isPC
    ? pcProfile?.spell_attack_bonus || getFinalSpellAttackBonus(pcProfile || {})
    : (combatant.spell_attack_bonus ??
      combatant.spell_attack_bonus_mod ??
      null);
  ...
  const conditionsLabel = displayConditions.length
    ? `Conditions (${displayConditions.length})`
    : "+ Conditions";
  const concentrationText = concentration
    ? concentrationSpellName || "Concentrating (Unlinked)"
    : "Not concentrating";
  ...
  async function handleToggleReaction() {
    ...
    if (isPC) {
      ...
      await supabase
        .from("combatants")
        .update({ reaction_used: !enemyReactionUsed })
        .eq("id", combatant.id);
    }
    onUpdate();
  }
  ...
  async function togglePcCondition(code) {
    ...
    const updated = pcConditions.includes(code)
      ? pcConditions.filter((c) => c !== code)
      : [...pcConditions, code];
    await supabase
  ...
  async function applyEnemyDamage(amount) {
    ...
    const isBloodiedNow =
      newHp > 0 && newHp <= Math.floor((combatant.hp_max ?? 0) / 2);
    const updatedConditions = nextZeroHpConditions(newHp, conditions);
    await supabase
      .from("combatants")
      .update({
        hp_current: newHp,
        public_status: isBloodiedNow ? "BLOODIED" : null,
    ...
    logCombat(
      ...
      "DM",
      "damage",
      `${combatant.name}: -${amount} HP (${oldHp} → ${newHp})`,
    );
    onUpdate();
  }
  ...
  async function removeCombatant() {
    ...
    logCombat(
      ...
      "DM",
      "remove",
      `${combatant.name} removed from encounter`,
    );
  ...
  return (
    <>
      <div
        className={`initiative-row initiative-row--density density-tier density-tier--${densityTier} ${isActive ? "active-turn" : ""} ${isNextUp ? "initiative-row--next-up" : ""}`}
        style={{
          display: "block",
          padding: "var(--init-card-pad, 10px)",
          borderColor: topBorder,
          background: cardBg,
          borderRadius: "var(--init-card-radius, 16px)",
        }}
      >
        <div
          className="initiative-top-band-grid"
          style={{
            display: "grid",
            gridTemplateColumns: "var(--init-top-col-width, 46px) minmax(0, 1fr) var(--init-top-col-width, 46px)",
            gap: "var(--init-top-gap, 7px)",
            alignItems: "stretch",
          }}
        >
          <StatBox
            label="Init"
            value={combatant.initiative_total ?? "—"}
            squareBand
            accent={
              isActive
                ? "var(--accent-blue)"
            ...
            onClick={isDM ? () => setShowInitiativeModal(true) : null}
          />
          <div
            style={{
              minWidth: 0,
              display: "flex",
              flexDirection: "column",
              gap: "var(--init-name-stack-gap, 1px)",
            }}
          >
            <div
              style={{
                fontSize: "var(--init-name-size, clamp(.98rem,2.05vw,1.3rem))",
                lineHeight: "var(--init-name-line-height, 1.02)",
                fontWeight: 800,
                color: "var(--text-primary)",
                minHeight: "var(--init-name-min-height, 34px)",
                display: "flex",
                alignItems: "center",
              }}
            >
            ...
            <div
              style={{
                display: "flex",
                gap: "var(--init-badge-gap, 5px)",
                flexWrap: "wrap",
                alignItems: "center",
                minHeight: "var(--init-badge-min-height, 18px)",
              }}
            >
              <span className={`badge badge-${combatant.side.toLowerCase()}`}>
                {sideLabel}
              </span>
              {isActive && (
                <span className="display-order-tag display-order-tag--active">
                  Current
                </span>
              )}
              {!isActive && isNextUp && (
                <span className="display-order-tag display-order-tag--next">
                  On Deck
                </span>
              )}
              {(enemyBloodied || pcBloodied) && (
                <span className="badge badge-bloodied">Bloodied</span>
              )}
              {isPC && playerState?.mage_armour_active && (
                <span className="initiative-inline-flag">Mage Armour</span>
              )}
            </div>
          </div>
          <StatBox
        ...
        <div
          style={{
            ...
            display: "flex",
            flexDirection: "column",
            gap: "var(--init-secondary-gap, 5px)",
          }}
        >
          <FullWidthStatusBar
            label="Reaction"
            value={rxUsed ? "Used" : "Available"}
            active={!rxUsed}
            onClick={canToggleReaction ? handleToggleReaction : null}
            accent={rxUsed ? "var(--accent-red)" : "var(--accent-green)"}
          />
          <FullWidthStatusBar
            label="Concentration"
            value={concentrationText}
            active={!!concentration}
            onClick={isPC && isDM ? handleTogglePcConcentration : null}
            accent="var(--accent-gold)"
          />
          {showHpBar ? (
            <InitiativeHeroHpBar
              current={isPC ? pcHpCurrent : enemyHpCurrent}
              max={isPC ? pcHpMax : enemyHpMax}
              tempHp={isPC ? tempHp : 0}
              bonusMaxHp={isPC ? pcBonusMaxHp : 0}
            />
          ) : isEnemy && !isDM ? (
          ...
          {conDc !== null && (
            <div
              className="con-check-banner con-check-banner--dm"
              style={{ marginTop: 1 }}
            >
          ...
          {isDM && showHpBar && (
            <div style={{ marginTop: -1 }}>
              {isPC ? (
                <InitiativeInlineDmgHeal
                  onDamage={applyPcDamage}
                  onHeal={applyPcHeal}
                />
              ) : (
                <InitiativeInlineDmgHeal
                  onDamage={applyEnemyDamage}
                  onHeal={applyEnemyHeal}
                />
              )}
            </div>
          )}
          {isDM && (
            <div style={{ display: "flex", flexDirection: "column", gap: "var(--init-dm-actions-gap, 6px)" }}>
              <button
                className="btn btn-ghost initiative-small-action"
                style={{
                  width: "100%",
                  minHeight: "var(--init-action-min-height, 34px)",
                  justifyContent: "center",
                }}
                onClick={(e) => {
                  e.stopPropagation();
                  setCondPickerOpen((p) => !p);
                }}
              >
                {conditionsLabel}
              </button>
              {isNonPC && (
                <button
                  ...
                  style={{
                    width: "100%",
                    minHeight: "var(--init-more-min-height, 30px)",
                    justifyContent: "center",
                  }}
                  onClick={(e) => {
                    e.stopPropagation();
            ...
            <div className="condition-picker">
              {CONDITIONS.map(({ code }) => (
                <button
                  ...
                  style={{
                    background: displayConditions.includes(code)
                      ? CONDITION_COLOURS[code]
                      : undefined,
                  }}
                  onClick={(e) => {
                    e.stopPropagation();
          ...
          {displayConditions.length > 0 && (
            <div className="initiative-chip-row" style={{ marginTop: -1 }}>
              {displayConditions.map((code) => (
                <span
                  ...
                  style={{
                    background:
                      CONDITION_COLOURS[code] || "var(--cond-default)",
                  }}
                >
                  {code}
                </span>
          ...
          {isDM && isNonPC && resPicker && (
            <div className="monster-dm-controls" style={{ marginTop: 2 }}>
              <InitiativeEnemySlotGrid
                combatant={combatant}
                onUpdate={onUpdate}
              />
              {(combatant.legendary_actions_max > 0 ||
                combatant.legendary_resistances_max > 0) && (
                <div className="initiative-secondary-block initiative-secondary-block--legendary">
                  {combatant.legendary_actions_max > 0 && (
                    <InitiativeLegendaryPips
                      label="LA"
                      max={combatant.legendary_actions_max}
                      used={combatant.legendary_actions_used ?? 0}
                      isDM={isDM}
                      onSpend={() =>
                        spendLegendary(
                          "legendary_actions_used",
                          combatant.legendary_actions_max,
                        )
                      }
                      onRestore={() =>
                        restoreLegendary("legendary_actions_used")
                      }
                  ...
                  {combatant.legendary_resistances_max > 0 && (
                    <InitiativeLegendaryPips
                      label="LR"
                      max={combatant.legendary_resistances_max}
                      used={combatant.legendary_resistances_used ?? 0}
                      isDM={isDM}
                      onSpend={() =>
                        spendLegendary(
                          "legendary_resistances_used",
                          combatant.legendary_resistances_max,
                        )
                      }
                      onRestore={() =>
                        restoreLegendary("legendary_resistances_used")
                      }
                      onReset={() =>
                        resetLegendary("legendary_resistances_used")
                      }
                      isActive={false}
                    />
                  )}
              ...
              {MODS.some((m) => (combatant[`mod_${m}`] ?? 0) !== 0) && (
                <div
                  style={{
                    display: "flex",
                    gap: 4,
                ...
                >
                  {MODS.map((mod) => {
                    const val = combatant[`mod_${mod}`] ?? 0;
                    return (
                      <div
                        key={mod}
                        style={{
                  ...
                  {DAMAGE_TYPES.map((type) => (
                    <button
                      ...
                      style={{
                        ...
                        borderRadius: 3,
                        cursor: "pointer",
                        border: `1px solid ${(combatant.resistances || []).includes(type) ? "var(--accent-blue)" : "var(--border)"}`,
                        background: (combatant.resistances || []).includes(type)
                          ? "rgba(74,158,255,0.2)"
                          : "var(--bg-panel-3)",
                        color: (combatant.resistances || []).includes(type)
                          ? "var(--accent-blue)"
                          : "var(--text-secondary)",
                      }}
                      onClick={() => toggleDamageType("resistances", type)}
                    >
                      {type}
                    </button>
                  ))}
                  ...
                  {DAMAGE_TYPES.map((type) => (
                    <button
                      ...
                      style={{
                        ...
                        borderRadius: 3,
                        cursor: "pointer",
                        border: `1px solid ${(combatant.immunities || []).includes(type) ? "var(--accent-gold)" : "var(--border)"}`,
                        background: (combatant.immunities || []).includes(type)
                          ? "rgba(240,180,41,0.15)"
                          : "var(--bg-panel-3)",
                        color: (combatant.immunities || []).includes(type)
                          ? "var(--accent-gold)"
                          : "var(--text-secondary)",
                      }}
                      onClick={() => toggleDamageType("immunities", type)}
                    >
                      {type}
                    </button>
                  ))}
              ...
              {(combatant.notes || formatClassLine(combatant)) && (
                <div className="monster-notes" style={{ marginBottom: 8 }}>
                  {[formatClassLine(combatant), combatant.notes]
                    .filter(Boolean)
                    .join(" • ")}
                </div>
              )}
              <button className="btn btn-danger" onClick={removeCombatant}>
                Remove
              </button>
            </div>
          )}
          {showBottomMeta && (
            ...
            >
              <MetaPill>PP {passivePerception ?? "—"}</MetaPill>
              <MetaPill>Spell DC {spellSave ?? "—"}</MetaPill>
              <MetaPill>
                Spell ATK{" "}
                {spellAttack !== null &&
      ...
      {showConPicker && isPC && playerState && pcProfile && (
        <ConcentrationSpellPickerModal
          open={showConPicker}
          profile={pcProfile}
          state={playerState}
          encounterId={encounterId}
          actor="DM"
          onClose={() => setShowConPicker(false)}
          onUpdate={onUpdate}
        />
      )}
      <InitiativeNumberModal
        open={showInitiativeModal}
        title={`Set initiative for ${combatant.name}`}
        defaultValue={String(combatant.initiative_total ?? "")}
...
function AddCombatantModal({ encounterId, onUpdate, onClose }) {
  ...
  async function addManual() {
    ...
    const basePayload = {
      ...
      ac: toInt(ac, 10),
      hp_max: toInt(hp, 10),
      hp_current: toInt(hp, 10),
      initiative_mod: toInt(mod, 0),
    };
    const enrichedPayload = compactObject({
      ...basePayload,
  ...
  return (
    <div className="modal-overlay" onClick={onClose}>
      <div
        className="modal-panel add-combatant-modal"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="modal-header">
          <div>
            ...
            <div className="modal-subtitle">
              Add from template or enter a manual combatant.
            </div>
          </div>
          <button
        ...
        <div style={{ display: "flex", gap: 8 }}>
          <button
            ...
            style={{
              flex: 1,
              borderColor:
                mode === "template" ? "var(--accent-blue)" : "var(--border)",
              color:
                mode === "template"
                  ? "var(--accent-blue)"
                  : "var(--text-secondary)",
          ...
          <button
            ...
            style={{
              flex: 1,
              borderColor:
                mode === "manual" ? "var(--accent-blue)" : "var(--border)",
              color:
                mode === "manual"
                  ? "var(--accent-blue)"
                  : "var(--text-secondary)",
          ...
          <>
            ...
            {templates.length === 0 && (
              <div className="empty-state">
                No templates yet. Add them in Manage.
              </div>
            )}
            <div className="add-combatant-template-list">
              {filteredTemplates.map((template) => (
                <div key={template.id} className="add-combatant-template-row">
                  <div
                    style={{
                      minWidth: 0,
                      display: "flex",
                  ...
                  >
                    <div style={{ minWidth: 0 }}>
                      <span
                        className={`badge badge-${(template.side || "enemy").toLowerCase()}`}
                        style={{ marginRight: 6 }}
                      >
                        {template.side || "ENEMY"}
                      </span>
                      ...
                      >
                        AC {template.ac} HP {template.hp_max}
                      </span>
                      {(template.legendary_actions_max > 0 ||
                        template.legendary_resistances_max > 0) && (
                        <span
                          style={{
                            fontSize: 10,
            ...
            <div className="form-row add-combatant-manual-grid">
              ...
              <div className="form-group">
                ...
                <input
                  className="form-input"
                  value={miniMarker}
                  onChange={(e) => setMiniMarker(e.target.value)}
                  placeholder="A / 1 / 🔴"
                  maxLength={8}
                />
              </div>
            ...
            <div className="form-row add-combatant-manual-grid">
              ...
              <div className="form-group">
                ...
                <input
                  className="form-input"
                  value={subclassName2}
                  onChange={(e) => setSubclassName2(e.target.value)}
                  placeholder="Optional"
                />
              </div>
              <div className="form-group">
                ...
                <input
                  ...
                  type="number"
                  value={classLevel2}
                  onChange={(e) => setClassLevel2(e.target.value)}
                  placeholder="Optional"
                />
              </div>
            </div>
