...
export default function JoinScreen({ onRoleSet }) {
  const [mode, setMode] = useState('choose'); // 'choose' | 'dm' | 'player' | 'display'
  ...
  return (
    <div className="join-screen">
      ...
      <div className="join-options">
        <button className="btn btn-primary btn-lg" onClick={() => setMode('dm')}>
          🎲 Dungeon Master
        </button>
        <button className="btn btn-ghost btn-lg" onClick={() => setMode('player')}>
          🧙 Player
        </button>
        <button className="btn btn-ghost btn-lg" onClick={() => setMode('display')}>
          📺 Display Screen
        </button>
      </div>
    </div>
...
function PlayerJoin({ setMode, onRoleSet }) {
  ...
  async function handleJoin() {
    ...
    try {
      ...
      if (dbError || !data) {
        throw new Error('Invalid join code. Check with your DM.');
      }

      joinAsPlayer(upperCode);
      // Store profile and encounter IDs for later use
...
function DisplayJoin({ setMode, onRoleSet }) {
  ...
  async function handleJoin() {
    ...
    try {
      ...
      if (dbError || !data) {
        throw new Error('Invalid display token. Ask your DM to generate one.');
      }
