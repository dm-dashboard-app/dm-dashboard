...
import { SHORT_REST_LOG_ACTION, SHORT_REST_RESPONSE_ACTION } from '../utils/shortRestWorkflow';
...
import WorldPanel from './dm/WorldPanel';
import { DISPLAY_MODE_OUT_OF_COMBAT, DISPLAY_MODE_IN_COMBAT, readDisplayModeFromEncounter, writeDisplayMode } from '../utils/displayMode';
...
function buildLongRestStatePatch(state = {}) {
  ...
  if (maxHp > 0 && Object.prototype.hasOwnProperty.call(state, 'current_hp')) patch.current_hp = maxHp;
  if (Object.prototype.hasOwnProperty.call(state, 'conditions')) patch.conditions = (state.conditions || []).filter(code => code !== 'UNC' && code !== 'PRN');
  ...
  if (Object.prototype.hasOwnProperty.call(state, 'spell_prep_ready')) patch.spell_prep_ready = false;
  for (let level = 1; level <= 9; level += 1) if (Object.prototype.hasOwnProperty.call(state, `slots_used_${level}`)) patch[`slots_used_${level}`] = 0;
  return patch;
}
...
export default function DMView() {
  ...
  const refreshAll = useCallback(async () => {
    ...
    const shortRestProcedureActiveNow = !!enc.data?.short_rest_active;
    const shortRestStartedAt = enc.data?.short_rest_started_at || null;
    let shortRestResponseLogs = [];
    if (shortRestProcedureActiveNow && shortRestStartedAt) {
      const { data } = await supabase
        .from('combat_log')
        .select('id, action, detail, created_at')
        .eq('encounter_id', encounterId)
        .eq('action', SHORT_REST_RESPONSE_ACTION)
        .gte('created_at', shortRestStartedAt)
        .order('created_at', { ascending: true })
        .limit(250);
      shortRestResponseLogs = data || [];
    }
    const responsesByStateId = {};
    shortRestResponseLogs.forEach((row) => {
      let detail = null;
      try {
        detail = typeof row.detail === 'object' ? row.detail : JSON.parse(row.detail);
      } catch (_err) {
        detail = null;
      }
    ...
    setShortRestProcedureActive(shortRestProcedureActiveNow);
    setShortRestResponsesByStateId(responsesByStateId);
  }, [encounterId]);
  ...
  const refreshActivityPresence = useCallback(async () => {
    ...
    const [rolls, alerts] = await Promise.all([
      supabase.from('secret_rolls').select('id', { count: 'exact', head: true }).eq('encounter_id', encounterId).gt('created_at', rollSince),
      supabase.from('concentration_checks').select('id', { count: 'exact', head: true }).eq('encounter_id', encounterId).eq('result', 'pending').gt('created_at', checkSince),
    ]);
  ...
  async function handleSetDisplayCombatMode(nextMode, source = 'manual') {
    if (!encounter?.id) return;
    const persisted = await writeDisplayMode(encounter.id, nextMode, source);
    setDisplayCombatMode(persisted);
    refreshAll();
  }
  ...
  async function executeLongRest() {
    if (!encounter) return;
    const { data: latestStates, error: latestStatesError } = await supabase.from('player_encounter_state').select('*, profiles_players(*), profiles_wildshape(form_name, hp_max)').eq('encounter_id', encounter.id);
    if (latestStatesError) return;
    const states = flattenStates(latestStates || []);
    const updateResults = await Promise.all(states.map(async state => supabase.from('player_encounter_state').update(buildLongRestStatePatch(state)).eq('id', state.id)));
    ...
    await supabase.from('encounters').update({ round: 1, turn_index: 0, long_rest_prep_active: false }).eq('id', encounter.id);
    ...
    setLongRestPrepOpen(false);
    await refreshAll();
    await refreshActivityPresence();
  }

  async function beginLongRest() {
    if (!encounter) return;
    if (!window.confirm('Long Rest — begin spell preparation and then restore all player HP, spell slots, and wild shape uses?')) return;
    const { data: latestStates, error: latestStatesError } = await supabase.from('player_encounter_state').select('*, profiles_players(*), profiles_wildshape(form_name, hp_max)').eq('encounter_id', encounter.id);
    if (latestStatesError) return;
    const states = flattenStates(latestStates || []);
    const affected = states.filter(state => hasPreparationRequirement(state?.profiles_players || {}));
  ...
  async function beginShortRest() {
    ...
    await supabase.from('encounters').update({ short_rest_active: true, short_rest_started_at: new Date().toISOString() }).eq('id', encounter.id);
  ...
  async function cancelShortRest() {
    if (!encounter || !shortRestProcedureActive) {
      setShortRestOpen(false);
      return;
    }
    if (!window.confirm('Cancel active short rest? Player responses for this cycle will be discarded.')) return;
    await supabase.from('encounters').update({ short_rest_active: false, short_rest_started_at: null }).eq('id', encounter.id);
    await supabase.from('combat_log').insert({ encounter_id: encounter.id, actor: 'DM', action: SHORT_REST_LOG_ACTION, detail: JSON.stringify({ type: 'cancel' }) });
    await supabase.from('combat_log').insert({ encounter_id: encounter.id, actor: 'DM', action: 'rest', detail: 'Short Rest cancelled — no healing or short-rest resource reset applied' });
    setShortRestOpen(false);
    await refreshAll();
  }

  async function cancelLongRestPrep() {
    if (!encounter) return;
    const targets = playerStates.filter(state => Object.prototype.hasOwnProperty.call(state || {}, 'spell_prep_ready'));
    await Promise.all(targets.map(async state => supabase.from('player_encounter_state').update({ spell_prep_ready: false }).eq('id', state.id)));
    await supabase.from('encounters').update({ long_rest_prep_active: false }).eq('id', encounter.id);
    setLongRestPrepOpen(false);
    await refreshAll();
  }
  ...
  async function handleRollEnemyInitiative() {
    ...
    if (targets.length === 0) {
      ...
      const persisted = await writeDisplayMode(encounter.id, DISPLAY_MODE_IN_COMBAT, 'initiative');
      setDisplayCombatMode(persisted);
  ...
  function handleTabSwitch(nextTab, event) {
    if (nextTab === 'activity' && encounterId) {
      markSeenBoundary(encounterId, 'rolls');
      markSeenBoundary(encounterId, 'checks');
      setRecentRollCount(0);
      setRecentAlertCount(0);
    }
    setTab(nextTab);
    event?.currentTarget?.blur();
    if (nextTab === 'activity') refreshActivityPresence();
  }
  ...
  const activityCount = recentRollCount + recentAlertCount;
  ...
  return (
    <div className="app-shell">
      ...
      <div style={dockShellStyle}>
        <div style={dockInnerStyle}>
          ...
          <div style={tabsRowStyle}>
            ...
            <button type="button" className="dm-bottom-tab-btn" style={tabButtonStyle(tab === 'activity')} onClick={event => handleTabSwitch('activity', event)}>Activity{activityCount > 0 ? <span className="tab-badge">{activityCount}</span> : ''}</button>
            <button type="button" className="dm-bottom-tab-btn" style={tabButtonStyle(tab === 'world')} onClick={event => handleTabSwitch('world', event)}>World</button>
            <button type="button" className="dm-bottom-tab-btn" style={tabButtonStyle(tab === 'manage')} onClick={event => handleTabSwitch('manage', event)}>Manage</button>
          </div>
      ...
      <ShortRestModal open={shortRestOpen} playerStates={playerStates} encounterId={encounter.id} responsesByStateId={shortRestResponsesByStateId} onClose={() => setShortRestOpen(false)} onCancelProcedure={cancelShortRest} shortRestProcedureActive={shortRestProcedureActive} onComplete={refreshAll} />
