...
export default function DMView() {
  ...
  useEffect(() => { if (encounterId) { refreshAll(); refreshActivityPresence(); } }, [encounterId, refreshAll, refreshActivityPresence]);
  ...
  async function handleLongRest() {
    ...
    const failedUpdate = updateResults.find(result => result.error); if (failedUpdate?.error) return;
    ...
    await refreshAll(); await refreshActivityPresence();
  }
  async function handleRollEnemyInitiative() {
    ...
    if (targets.length === 0) {
      ...
      setRollingInit(true);
      await Promise.all(reroll.map(c => { const roll = Math.floor(Math.random() * 20) + 1 + (c.initiative_mod ?? 0); return supabase.rpc('set_initiative', { p_combatant_id: c.id, p_total: roll }); }));
      await supabase.from('encounters').update({ round: 1, turn_index: 0 }).eq('id', encounter.id); setRollingInit(false); refreshAll(); return;
    }
    setRollingInit(true);
    await Promise.all(targets.map(c => { const roll = Math.floor(Math.random() * 20) + 1 + (c.initiative_mod ?? 0); return supabase.rpc('set_initiative', { p_combatant_id: c.id, p_total: roll }); }));
    await supabase.from('encounters').update({ round: 1, turn_index: 0 }).eq('id', encounter.id); setRollingInit(false); refreshAll();
  }
  ...
  if (!encounter) return <div className="app-shell"><div className="shell-nav-stack"><div className="top-bar"><div className="top-bar-spacer" /></div></div><div className="main-content dm-main-content"><EncounterSetup onEncounterCreated={enc => { setEncounter(enc); setEncounterId(enc.id); setTab('combat'); }} /><ManagementScreens currentEncounter={null} displayToken={null} joinCodes={[]} onGenerateDisplayToken={null} onRevokeDisplayToken={null} onFrontScreen={null} onSignOut={signOut} /></div></div>;
  ...
  const bottomActionHeight = 86;
  const bottomTabHeight = 62;
  const alertOffset = bottomActionHeight + bottomTabHeight + 10;

  return (
    <div className="app-shell">
      <div className="shell-nav-stack"><div className="top-bar"><div className="top-bar-spacer" /></div></div>
      <DMNotificationOverlay encounterId={encounter.id} docked bottomOffset={alertOffset} />
      <div className="main-content dm-main-content main-content--with-bottom-dock" style={{ paddingBottom: 230 }}>
        ...
        {tab === 'players' && <DMPlayerCardsSection combatants={pcCombatants} playerStates={playerStates} encounterId={encounter.id} playerEditMode={encounter.player_edit_mode} onUpdate={refreshAll} title="Players" subtitle={`${pcCombatants.length} player${pcCombatants.length === 1 ? '' : 's'} in encounter`} />}
      ...
      <div style={{ position: 'fixed', left: 0, right: 0, bottom: bottomActionHeight, zIndex: 131, padding: '8px 12px 0', background: 'linear-gradient(180deg,rgba(17,22,29,0),rgba(17,22,29,.72) 34%,rgba(17,22,29,.98))', borderTop: '1px solid rgba(74,158,255,.08)' }}>
        <div className="dm-bottom-action-row" style={{ width: 'min(1180px,calc(100vw - 24px))' }}>
