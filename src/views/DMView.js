...
export default function DMView() {
  ...
  async function handleRollEnemyInitiative() {
    ...
    if (targets.length === 0) {
      ...
      setRollingInit(true);
      await Promise.all(reroll.map(c => supabase.rpc('set_initiative', { p_combatant_id: c.id, p_total: Math.floor(Math.random() * 20) + 1 + (c.initiative_mod ?? 0) })));
      await supabase.from('encounters').update({ round: 1, turn_index: 0 }).eq('id', encounter.id);
      setRollingInit(false); refreshAll(); return;
    }
    setRollingInit(true);
    await Promise.all(targets.map(c => supabase.rpc('set_initiative', { p_combatant_id: c.id, p_total: Math.floor(Math.random() * 20) + 1 + (c.initiative_mod ?? 0) })));
    await supabase.from('encounters').update({ round: 1, turn_index: 0 }).eq('id', encounter.id);
    setRollingInit(false); refreshAll();
  }
  ...
  return (
    <div className="app-shell">
      ...
      <div className="main-content dm-main-content main-content--with-bottom-dock">
        {tab === 'combat' && <div className="dm-combat-layout dm-combat-layout--enhanced"><div className="dm-initiative-column"><div className="initiative-top-bar"><div className="initiative-top-bar-primary">Round {encounter.round}</div></div><InitiativePanel encounter={encounter} combatants={combatants} playerStates={playerStates} role="dm" onUpdate={refreshAll} /></div><div className="dm-combat-side-column"><DMPlayerCardsSection combatants={pcCombatants} playerStates={playerStates} encounterId={encounter.id} playerEditMode={encounter.player_edit_mode} onUpdate={refreshAll} title="Party Status" subtitle={`${pcCombatants.length} player${pcCombatants.length === 1 ? '' : 's'} in encounter`} /></div></div>}
      ...
      <div className="dm-bottom-action-bar"><div className="dm-bottom-action-row"><button className="btn btn-ghost" onClick={() => setShortRestOpen(true)}>Short Rest</button><button className="btn btn-ghost" onClick={handleLongRest}>Long Rest</button><button className="btn btn-ghost" onClick={handleRollEnemyInitiative} disabled={rollingInit}>{rollingInit ? 'Rolling…' : 'Initiative'}</button><button className="btn btn-primary dm-bottom-action-next" onClick={handleNextTurn}>▶ Next</button></div></div>
