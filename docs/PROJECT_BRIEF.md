...
- source-split item conversion now emits attunement-required minimal mechanics payloads for inventory-slot attunement items (for example Pearl of Power and dragon vessel tiers), so runtime no longer falls back to equip-only behavior for those rows
...
- source-split conversion now also adds deterministic generated canonical ordinary +1/+2/+3 weapon/armor/shield families from trusted PHB mundane base rows already in the pipeline, with explicit generated provenance metadata, explicit ammunition/non-table-weapon exclusion, and no broad magical-variants monolith ingestion
...
- inventory/runtime and import-lane follow-up now hardens inventory snapshot item normalization to render-safe primitive fields (blocking malformed-row black-screen regressions on player inventory open), and 5etools import meta/RPC validation now supports the strict active two-layer lane (`5etools_items_by_source_curated` + `5etools_items_by_source_curated_generated_canonical_enhancements`) with expected-row parity checks preserved
- Manage → Imports review/report UX now uses mobile-first stacked summary/report blocks with wrap-safe control grids and a controlled expansion toggle for the full imported-row dump, so large import reports remain scannable on narrow phone viewports
