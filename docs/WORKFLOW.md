...
### 3.4.1 Mandatory support-doc update on implementation PRs

Every implementation PR must update project-memory docs as part of the same batch when roadmap/current-state/process content changed, so done vs not-done state stays reliable across sessions.
...
PR-body completion-log prose is optional.

Merge-readiness gate: an implementation PR is **not merge-ready** if required support-doc updates are missing from that same PR.
...
PR templates may include optional summary sections, but repo docs remain the durable continuity record.

Do not treat PR-body prose sections as merge blockers by themselves.
